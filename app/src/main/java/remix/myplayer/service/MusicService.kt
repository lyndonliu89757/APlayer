...
//    Timber.v("onSharedPreferenceChanged, key: $key")
    ...
    private fun setUp() {
        ...
        notify =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && !settingPrefs.classicNotify) {
                NotifyImpl24(this)
            } else {
                NotifyImpl(this)
            }
...
//    throw IllegalStateException("onEnded")
        ...
        when (action) {
            ...
            ACTION_SHORTCUT_MYLOVE -> {
                tryLaunch {
                    ...
                    val songs =
                        withContext(Dispatchers.IO) {
                            songRepository.getSongsByModels(
                                listOf(
                                    playlist
                                )
                            )
    ...
    private fun handleMusicEvent(intent: Intent?) {
        ...
        when (intent.action) {
            ...
            PERMISSION_CHANGE -> onPermissionChanged(
    ...
    private fun updateMediaSession(control: Int) {
        ...
        if (!updatePlayStateOnly(control)) {
            val placeholder = R.drawable.album_empty_bg_day
            Glide.with(this)
                .asBitmap()
                .load(currentSong)
                .error(placeholder)
...
//        }
                ...
                playback.prepare(
                    song,
                    if (playModel == MODE_REPEAT) song else playQueue.nextSong,
                    offset
                )
