...
sealed class Song(
  ...
  fun copy(): Song {
    return when (this) {
      ...
      is Remote -> {
        Remote(title, album, artist, _duration, data, size, year, _genre ?: "", track, dateCreated, account, pwd)
      }
  ...
  class Remote(
    ...
    -abs(data.hashCode().toLong()), title, title, album, 0L, artist, 0L, duration, data, size, year, genre, track, dateCreated
  ) {
    val headers by lazy {
      mapOf(
    ...
    constructor(title: String, data: String, size: Long, dateCreated: Long, account: String, pwd: String) : this(
      title,
      "",
      "",
      0L,
